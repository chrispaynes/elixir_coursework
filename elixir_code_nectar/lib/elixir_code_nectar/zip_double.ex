defmodule ElixirCodeNectar.ZipDouble do
  ...
  Store an array of numbers to a variable and create a function to multiply
  each number in the array by a given integer argument. Return an array with
  the original numbers and the doubled results as key-value pairs in an object.
  ...
  @doc """
  Each run function merges a list with a copy of the list multiplied by a
  multiplier argument. Returns a list of tuples where the key is the original
  list number and the value is the number's multiplied value.
  """
  @spec run(list, integer) :: list
  def run(list, multiplier) do
    # Multiplied_list stores the list array with each value multiplied
    # by a multiplier.
    multiplied_list =
      list |> Enum.map(&(&1 * multiplier))

    # Enum.zip joins the list and the multiplied list into a list of merged
    # tuples with key/value pairs.
    Enum.zip(list, multiplied_list)
  end

  @spec run2(list, integer) :: list
  def run2(list, multiplier) do
    # Multiplied_list stores the list array with each value multiplied
    # by a multiplier.
    multiplied_list = Stream.map(list, fn(x) -> x * multiplier end)

    # Stream.zip joins the list and the multiplied list into a list of merged
    # tuples with key/value pairs.
    # Enum.lo_list evaluates the stream into a list.
    Stream.zip(list, multiplied_list) |> Enum.to_list
  end

  @spec run3(list, integer) :: list
